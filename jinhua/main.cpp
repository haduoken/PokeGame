...
double shenglv(card a1, card a2, card a3,int nplayers)
{
	bool cardmap[52];
	for (int h = 0; h < 52; h++)
		cardmap[h] = true;
	...
	while (nplayers--)
	{
		ret *= (big - nplayers) / (big + small - nplayers);
	}
	return ret;
}
void judge(int moneydown, int moneyfollow,card a1,card a2,card a3,int nplayers)
{
	double snlv = shenglv(a1, a2, a3, nplayers);
	double qiwang;
	qiwang = snlv*(moneydown + moneyfollow) - (1 - snlv)*(moneydown);
	if (qiwang >= 0)
	{
		cout << "this turn you should follow and you're about to win " << qiwang << " ";
	}
	else cout << "this turn you should drop " << qiwang << " ";
}
int main()
{
	card a1(heitao,4);
